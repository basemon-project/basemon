...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("basemon", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("basemonMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("basemonMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("basemonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("basemon-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("basemon", "Running basemonMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
