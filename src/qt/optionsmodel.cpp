...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::basemon);
    ...
    if (!settings.contains("nAnonymizebasemonAmount"))
        settings.setValue("nAnonymizebasemonAmount", 1000);

    nAnonymizebasemonAmount = settings.value("nAnonymizebasemonAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizebasemonAmount"))
        SoftSetArg("-anonymizebasemonamount", settings.value("nAnonymizebasemonAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in basemon.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizebasemonAmount:
            return QVariant(nAnonymizebasemonAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizebasemonAmount:
            nAnonymizebasemonAmount = value.toInt();
            settings.setValue("nAnonymizebasemonAmount", nAnonymizebasemonAmount);
            emit anonymizebasemonAmountChanged(nAnonymizebasemonAmount);
            break;
        case CoinControlFeatures:
